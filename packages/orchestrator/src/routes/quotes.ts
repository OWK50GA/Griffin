...
const validateQuoteRequest = [
  ...
  body('slippageTolerance').optional().isFloat({ min: 0, max: 1 }).withMessage('Valid slippage tolerance (0-1) required')
];

// POST /api/v1/quotes - Get payment quotes
router.post('/', validateQuoteRequest, asyncHandler(async (req: Request, res: Response) => {
